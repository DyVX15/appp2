...
let kullanici = message.mentions.users.first()
    ...
    .setDescription(`Çok hızlısın lütfen 5 saniye bekle <@${message.author.id}>.`)).then(msg => msg.delete({ timeout: 8000, reason: 'Yapılması gerekiyordu.' }));
        ...
        if(!member) {
                const Tooblek = new Discord.MessageEmbed()
                        .setDescription(`Seveceğin kişiyi etiketlemelisin <@${message.author.id}>.`)
                        .setColor("RANDOM")
        ...
        if(!kullanici.bot == false) {
                const Tooblek = new Discord.MessageEmbed()
                        .setDescription(`Botu sevemezsin <@${message.author.id}>.`)
                        .setColor("RANDOM")
                message.channel.send(Tooblek);
                return
        }
    ...
    .setAuthor('')
    .setColor("RANDOM")
    .setImage(`https://cdn.discordapp.com/attachments/855490846563762187/856233142292971520/796fdda81b2e0eb8bf8028b8893f6e3c.gif`)
    .setDescription(`${s}, ${member.user} isimli kişiyi sevdi.`)
