...
let kullanici = message.mentions.users.first()
    ...
    .setDescription(`Çok hızlısın lütfen 5 saniye bekle <@${message.author.id}>.`)).then(msg => msg.delete({ timeout: 8000, reason: 'Yapılması gerekiyordu.' }));
        ...
        if(!member) {
                const Tooblek = new Discord.MessageEmbed()
                        .setDescription(`Sarılacağın kişiyi etiketlemelisin <@${message.author.id}>.`)
                        .setColor("RANDOM")
        ...
        if(!kullanici.bot == false) {
                const Tooblek = new Discord.MessageEmbed()
                        .setDescription(`Bota sarılamazsın <@${message.author.id}>.`)
                        .setColor("RANDOM")
                message.channel.send(Tooblek);
                return
        }
    ...
    .setAuthor('')
    .setColor("RANDOM")
    .setImage(`https://cdn.discordapp.com/attachments/855490846563762187/856231051516575754/love-hug-icegif.gif`)
    .setDescription(`${s}, ${member.user} isimli kişiye sarıldı.`)
